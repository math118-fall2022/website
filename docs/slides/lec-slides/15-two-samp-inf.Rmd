...
Begin by writing each observed value `jitter` on its own card, for a total of `r nrow(parkinsons)` cards. Then:

--

1. Shuffle the cards well and deal `r sum(parkinsons$status == "Healthy")` into one group, and the remaining `r sum(parkinsons$status == "PD")` into a second group
