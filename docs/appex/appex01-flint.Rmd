...
We will rely heavily on the `tidyverse` package. You can check if you have it installed by running the following in your console:

```{r eval = F}
library(tidyverse)
```

If RStudio tells you the package cannot be found, then install it by running the following in your **console**:
...
Below are side-by-side box plots for the three flushing times in each of the five zip codes we considered. Add `x` and `y` labels along with a title inside the `labs()` function.

```{r}
ggplot(data = flint_focus, aes(x = factor(zip), y = lead)) +
  geom_boxplot(aes(fill = factor(draw))) +
  labs(x = "____", y = "____", fill = "Flushing time",
       title = "____") +
...
**Once you have completed the activity, knit your document one last time, then push your final changes to your GitHub repo! Make sure your repo is updated on GitHub, and that's all you need to do to submit application exercises for participation.**
